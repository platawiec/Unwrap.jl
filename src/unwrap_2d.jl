...
function calculate_reliability(pixel_image, params)
    ...
    # inner loop
    for i in CartesianRange(CartesianIndex(2,2), CartesianIndex(size(A)[1]-1, size(A)[2]-1))
...
function calculate_reliability_horizontal_edges(pixel_image, params)
    ...
    for i in CartesianRange(size(edges_horizontal))
        edges_horizontal.pixel_1 = pixel_image[i]
        edges_horizontal.pixel_2 = pixel_image[i+CartesianIndex(1, 0)]
        edges_horizontal.reliability = (pixel_image[i].reliability
                                        + pixel_image[i+CartesianIndex(1, 0)].reliability)
        edges_horizontal.periods = find_period(pixel_image[i].val,
...
function calculate_reliability_vertical_edges(pixel_image, params)
    ...
    for i in CartesianRange(size(edges_vertical))
        edges_vertical.pixel_1 = pixel_image[i]
        edges_vertical.pixel_2 = pixel_image[i+CartesianIndex(0, 1)]
        edges_vertical.reliability = (pixel_image[i].reliability
                                      + pixel_image[i+CartesianIndex(0, 1)].reliability)
        edges_vertical.periods = find_period(pixel_image[i].val,
